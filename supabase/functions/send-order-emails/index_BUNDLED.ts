...
async function sendEmail(to: string, subject: string, html: string, replyTo?: string) {
    ...
    try {
        const response = await fetch('https://api.resend.com/emails', {
            ...
            body: JSON.stringify({ from: `WAMOCON Academy <${EMAIL_FROM}>`, to: [to], subject, html, ...(replyTo && { replyTo }) }),
        })
        const data = await response.json()
        if (!response.ok) { console.error('Resend Error:', data); return { success: false, error: data.message } }
        return { success: true, id: data.id }
    } catch (error) { console.error('Sending Error:', error); return { success: false, error: error.message } }
}
...
serve(async (req) => {
    ...
    try {
        ...
        // Prepare HTML (Inject Logo URL correctly)
        const logoUrl = `${DOMAIN}/logo/WAMOCON_ACADEMY_LOGO.png`;
        let customerHtml = replaceTemplateVariables(TEMPLATE_CUSTOMER.replace('\${LOGO_URL}', logoUrl), templateData);
        let adminHtml = replaceTemplateVariables(TEMPLATE_ADMIN, templateData);

        // Replace placeholder domain in case it was missed
        customerHtml = customerHtml.replace('https://your-domain.com/logo/WAMOCON_ACADEMY_LOGO.png', logoUrl);
        ...
        const results = {
            customer: res1.status === 'fulfilled' ? res1.value : res1.reason,
            admin: res2.status === 'fulfilled' ? res2.value : res2.reason
        }
